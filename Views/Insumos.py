...
from Controllers.Insumos_dao import crear_tabla_insumos, borrar_tabla, Insumo, guardar, listar, editar, eliminar
...
class InsumosFrame(tk.Toplevel):
    ...
    def setup_ui(self):
        self.label_proveedor = tk.Label(self, text='ID Proveedor:')
        self.label_proveedor.config(font=('Arial', 12, 'bold'))
        self.label_proveedor.grid(row=0, column=0, padx=10, pady=10)
        ...
        self.entry_proveedor.grid(row=0, column=1, padx=10, pady=10, columnspan=2)
        ...
        self.entry_precio.grid(row=3, column=1, padx=10, pady=10, columnspan=2)
        ...
        self.tabla = ttk.Treeview(self, columns=('Proveedor', 'Codigo', 'Nombre', 'Precio', 'Cantidad'))
        self.tabla.grid(row=6, column=0, columnspan=4, sticky='nsew')
        self.scroll = ttk.Scrollbar(self, orient='vertical', command=self.tabla.yview)
        self.scroll.grid(row=6, column=4, sticky='ns')
        self.tabla.configure(yscrollcommand=self.scroll.set)

        self.tabla.heading('#0', text='#')
        self.tabla.heading('#1', text='ID Proveedor')
        ...
        self.button_salir.grid(row=7, column=2, padx=10, pady=10)
    ...
    def validar_datos(self):
        ...
        # Validar que el ID del proveedor sea un entero
        try:
            int(proveedor)
        except ValueError:
            messagebox.showerror("Error de Validación", "El ID del proveedor debe ser un número entero.")
            return False

        # Validar que el código sea una cadena alfanumérica
        if not codigo.isalnum():
            messagebox.showerror("Error de Validación", "El código debe ser una cadena alfanumérica.")
            return False
        ...
        # Validar que el precio sea un flotante
        try:
            float(precio)
        except ValueError:
            messagebox.showerror("Error de Validación", "El precio debe ser un número flotante.")
            return False
    ...
    def editar_datos(self):
        try:
            ...
            self.entry_proveedor.insert(0, values[0])
    ...
    def actualizar_datos(self, item, old_values):
        ...
        nuevos_valores = {
            ...
            'codigo': self.mi_codigo.get(),
            'nombre': self.mi_nombre.get(),
            'precio': float(self.mi_precio.get()),
            'cantidad': int(self.mi_cantidad.get()),
        }
        
        # Solo actualizar los campos que han cambiado
        datos_actualizados = {}
        campos = ['proveedor', 'codigo', 'nombre', 'precio', 'cantidad']
